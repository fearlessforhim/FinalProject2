...
void hashmapIter(Hashmap* map, HashmapIter* iter_ret) {
	iter_ret->begin = 1;
	iter_ret->end = 0;
	for(iter_ret->bucket = 0; iter_ret->bucket < map->bucketCount; ++iter_ret->bucket) {
		iter_ret->entry = map->buckets[iter_ret->bucket];
		if(iter_ret->entry != NULL)
			break;
	}
	if(iter_ret->entry == NULL) {
		iter_ret->end = 1;
	...
	Entry* entry = (Entry*) iter_ret->entry;
	iter_ret->key = entry->key;
	iter_ret->value = entry->value;
}

void hashmapNext(Hashmap* map, HashmapIter* iter) {
	if(iter->end)
		return;
	iter->begin = 0;
	Entry* entry = (Entry*) iter->entry;
	iter->entry = entry->next;
	if(iter->entry == NULL) {
		for(iter->bucket += 1; iter->bucket < map->bucketCount; ++iter->bucket) {
    	iter->entry = map->buckets[iter->bucket];
			if(iter->entry != NULL)
				break;
		}
		if(iter->entry == NULL) {
